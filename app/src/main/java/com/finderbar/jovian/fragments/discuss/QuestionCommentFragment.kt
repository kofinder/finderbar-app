...
import com.finderbar.jovian.datasource.convertComment
...
import query.AllCommentByQuestionQuery
import subscription.CommentSubscription


class QuestionCommentFragment: Fragment(), OnCvItemClick {
    ...
    private var queryApi: ApolloCall<AllCommentByQuestionQuery.Data>? = null
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        binding.btnSent.setOnClickListener {
            dialog.show()
            askAnswerVm.saveQuestionComment(discussId!!, binding.edInputBody.text.toString())
        }

        askAnswerVm.result.observe(this, Observer {
            binding.edInputBody.clearFocus()
            binding.edInputBody.text?.clear()
            dialog.dismiss()
        ...
        // get data
        getAllComment(InputCriteria(discussId!!, "", 20, 0), binding.recyclerView)
    ...
    private fun getAllComment(criteria: InputCriteria, recyclerView: RecyclerView) {
        val query = AllCommentByQuestionQuery.builder().criteria(criteria.get()).build()
        queryApi = apolloClient.query(query).responseFetcher(ApolloResponseFetchers.NETWORK_FIRST)
        queryApi?.enqueue({
            adaptor.addAll(convertComment(it.data()?.allCommentByQuestion()?.comments()))
            if(!adaptor.isEmpty()) {
