...
class AnswerActionDialogFragment : DialogFragment() {
    ...
    private fun setupViewPager(viewPager: ViewPager) {
        ...
        adapter.addFrag(AnswerCommentFragment(), "comment", discussId!!)
        adapter.addFrag(AnswerFavoriteFragment(), "like", discussId!!)
        adapter.addFrag(AnswerUpVoteFragment(), "upvote", discussId!!)
        adapter.addFrag(AnswerDownVoteFragment(), "downvote", discussId!!)
        viewPager.adapter = adapter
    }


    companion object {
        const val TAG = "AnswerActionDialogFragment"
        fun newInstance(_id: String): AnswerActionDialogFragment {
            val fragment = AnswerActionDialogFragment()
            val args = Bundle()
            args.putString(AppConstants.ARG_KEY_DISCUSS_ID, _id)
